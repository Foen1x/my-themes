...
<template>
  <div>
    ...
    <section v-if="tab === 'dashboard'" class="pattern-canvas">
      ...
      <div class="metric-row"><div><span>Coverage</span><strong>82/82</strong><small>All routes active</small></div><div><span>Open reviews</span><strong>7</strong><small>2 due today</small></div><div><span>Theme checks</span><strong>96%</strong><small>Across 10 palettes</small></div></div>
    ...
    <section v-else class="pattern-canvas feedback-pattern"><el-result icon="success" title="Catalog check complete" sub-title="82 component routes and ten semantic themes passed validation."><template #extra><el-button type="primary">View report</el-button><el-button>Run again</el-button></template></el-result><div class="feedback-list"><el-alert title="No missing previews" type="success" show-icon /><el-alert title="Two color pairs are close to the review threshold" type="warning" show-icon /><el-alert title="Bundle report is available" type="info" show-icon /></div></section>
  </div>
</template>
